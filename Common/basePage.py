...
class BasePage(object):
    ...
    def __init__(self, driver):
        ...
        self.driver = driver
        # 日清首页链接
        self.riqing_index_url = "http://10.189.0.78/warning-service/"
        # 姓名输入框
    ...
    def click_element(self, loc, model=None):
        ...
        try:
            ele.click()
            self.logger.info(f'点击"{model}",点击成功')
        except:
    ...
    def switch_iframe(self, frame_refer, timeout=30, poll_frequency=0.5, model=None):
        # 等待 iframe 存在
        self.logger.info(f'iframe 切换操作:{model}')
        try:
            # 切换 == index\name\id\WebElement
    ...
    def loginRQ(self):
        ...
        self.driver.get(self.riqing_index_url)
        self.logger.info(f"打开链接: {self.riqing_index_url}...")
        # 窗口最大化
    ...
    def setAttribute(self, elementObj, attributeName, value):
        ...
        :attributeName属性名字
        :value属性值
        :arguments[0]获取elementObj的值
        :arguments[1]获取value的值
        :%s获取attributeName的值
        """
